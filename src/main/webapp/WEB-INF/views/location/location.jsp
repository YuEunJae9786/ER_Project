...
<div class="mainBody">
	<!-- 지도를 표시할 div 입니다 -->
	<div id="map"
		style="width: 60%; height: 600px; margin-right: 1%; margin-left: 4%; box-sizing: border-box;"></div>
	<!-- 지도 js -->
	<script type="text/javascript"
		src="//dapi.kakao.com/v2/maps/sdk.js?appkey=4306199893f86fa673fc035ff7b9639a"></script>
	<script>
			var markers = [];
			...
			var normalImage = overMarkerImage(markerSize), overImage = overMarkerImage(overMarkerSize), clickImage = createMarkerImage(markerSize);
			...
			function addMarker(position) {
				...
				kakao.maps.event.addListener(marker, 'mouseover', function() {
					...
					if (!selectedMarker || selectedMarker !== marker
							|| selectedMarker.getImage() !== clickImage) {
			...
			function createMarkerImage(markerSize) {
				var markerImage = new kakao.maps.MarkerImage(imageClick, // 스프라이트 마커 이미지 URL
				markerSize // 마커의 크기
				);
			...
			function overMarkerImage(markerSize) {
				var overImage = null;
				var markerImage = new kakao.maps.MarkerImage(imageNormal,
						markerSize // 마커의 크기
				);
			...
			<c:forEach var="i" items="${getKickList}">
				<li id="list">
					...
					<div id="detail${i}" class="kick-info-detail">
						<div class="detail-fix">
							<div class="detail-info">
								...
								<div
									style="border-bottom: 0.5px solid #ccc; padding: 10px; margin-bottom: 10px;">
									<div>좌표 정보</div>
									<div style="padding: 5px;">좌표
										(${list[i-1].stationLatitude}, ${list[i-1].stationLongitude})</div>
								</div>
							</div>
							<img
								src="//thumbnail10.coupangcdn.com/thumbnails/remote/48x48ex/image/vendor_inventory/08f3/40b6e6c7e6086d690435883a6fa0a6d71dbd6751713617c3b1203e906240.png"
						...
						<div class="spec">
							상세정보 <br> 스팩1.<br> 스팩2.<br> 스팩3.<br> <a>더많은
								정보 보러가기(여기에는 링크를 걸어준다.)</a>
						</div>
						<div>
...
<script>
	        $(document).ready(function(data) {
				...
				function getFetch() {
					fetch(
							"http://openapi.seoul.go.kr:8088/4f4975567873657534395075496a44/json/bikeList/1/10/")
							.then(function(response) {
								if (response.status != 200) {
									...
									function(bikeList) {
										var bikeList = bikeList["rentBikeStatus"]["row"];
										for (var i = 0; i < bikeList.length; i++) {

											position = {
												title : 'list' + i,
												latlng : new kakao.maps.LatLng(
														bikeList[i].stationLatitude,
														bikeList[i].stationLongitude)
											}
											addMarker(position);
										}
